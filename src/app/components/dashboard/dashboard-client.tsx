...
function FeedbackButtons({ agentId, messageContent, conversationId }: { agentId: string; messageContent: string; conversationId: string | null }) {
  ...
  const sendFeedback = async (type: "thumbs_up" | "thumbs_down") => {
    ...
    setFeedback(type);
    try {
      await fetch("/api/evals/feedback", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ agentId, feedback: type, messageContent, conversationId }),
      });
    } catch { /* best-effort */ }
  };
  ...
  return (
    <div className="flex items-center gap-1 mt-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
      <button
        ...
        className={`p-1 rounded text-xs transition-colors ${
          feedback === "thumbs_up"
            ...
            : "text-neutral-400 hover:text-secondary hover:bg-secondary/5"
        }`}
        title="Good response"
      >
        <svg className="w-3.5 h-3.5" fill={feedback === "thumbs_up" ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      <button
        ...
        title="Poor response"
      >
        <svg className="w-3.5 h-3.5" fill={feedback === "thumbs_down" ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    <div className="h-screen flex bg-surface">
      ...
      <aside className="w-72 bg-primary text-surface flex flex-col shrink-0">
        ...
        <div className="px-5 py-3 border-t border-neutral-800 space-y-1">
          ...
          <a href={`/dashboard/${company.id}/chai-time`} className="flex items-center gap-2 text-xs text-neutral-400 hover:text-neutral-200 transition-colors py-1">
            <svg className="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M17 8h1a4 4 0 110 8h-1"/><path d="M3 8h14v9a4 4 0 01-4 4H7a4 4 0 01-4-4V8z"/><line x1="6" y1="2" x2="6" y2="4"/><line x1="10" y1="2" x2="10" y2="4"/><line x1="14" y1="2" x2="14" y2="4"/></svg>
            Chai Time
          </a>
          ...
          <a href={`/dashboard/${company.id}/workflows`} className="flex items-center gap-2 text-xs text-neutral-400 hover:text-neutral-200 transition-colors py-1">
            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" /></svg>
            Workflows
          </a>
          <a href={`/dashboard/${company.id}/search`} className="flex items-center gap-2 text-xs text-neutral-400 hover:text-neutral-200 transition-colors py-1">
          ...
          <a href={`/dashboard/${company.id}/evals`} className="flex items-center gap-2 text-xs text-neutral-400 hover:text-neutral-200 transition-colors py-1">
            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.562.562 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.562.562 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" /></svg>
            Agent Quality
          </a>
          <a href={`/dashboard/${company.id}/leaderboard`} className="flex items-center gap-2 text-xs text-neutral-400 hover:text-neutral-200 transition-colors py-1">
            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 18.75h-9m9 0a3 3 0 013 3h-15a3 3 0 013-3m9 0v-3.375c0-.621-.503-1.125-1.125-1.125h-.871M7.5 18.75v-3.375c0-.621.504-1.125 1.125-1.125h.872m5.007 0H9.497m5.007 0a7.454 7.454 0 01-.982-3.172M9.497 14.25a7.454 7.454 0 00.981-3.172M5.25 4.236c-.982.143-1.954.317-2.916.52A6.003 6.003 0 007.73 9.728M5.25 4.236V4.5c0 2.108.966 3.99 2.48 5.228M5.25 4.236V2.721C7.456 2.41 9.71 2.25 12 2.25c2.291 0 4.545.16 6.75.47v1.516M18.75 4.236c.982.143 1.954.317 2.916.52A6.003 6.003 0 0116.27 9.728M18.75 4.236V4.5c0 2.108-.966 3.99-2.48 5.228m0 0a6.023 6.023 0 01-3.52 1.122 6.023 6.023 0 01-3.52-1.122" /></svg>
            Leaderboard
          </a>
          <a href={`/dashboard/${company.id}/templates`} className="flex items-center gap-2 text-xs text-neutral-400 hover:text-neutral-200 transition-colors py-1">
          ...
          <a href={`/dashboard/${company.id}/integrations`} className="flex items-center gap-2 text-xs text-neutral-400 hover:text-neutral-200 transition-colors py-1">
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4 bg-gradient-to-b from-surface to-white">
              ...
              {messages.map((msg, msgIndex) => (
                <div
                  key={msg.id}
                  className={`flex animate-fade-in-up group ${
