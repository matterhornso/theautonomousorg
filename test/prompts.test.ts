...
describe("System Prompts", () => {
  it("includes company name and industry", async () => {
    const prompt = await buildSystemPrompt("Sales", mockAnalysis, roster);
  ...
  it("includes role-specific instructions", async () => {
    const salesPrompt = await buildSystemPrompt("Sales", mockAnalysis, roster);
    expect(salesPrompt).toContain("BANT");
    expect(salesPrompt).toContain("pipeline");

    const mktgPrompt = await buildSystemPrompt("Marketing", mockAnalysis, roster);
  ...
  it("lists other agents in roster", async () => {
    const prompt = await buildSystemPrompt("Sales", mockAnalysis, roster);
  ...
  it("handles unknown role gracefully", async () => {
    const prompt = await buildSystemPrompt("Unknown Role", mockAnalysis, roster);
    expect(prompt).toContain("Acme Corp");
    expect(prompt).toContain("Unknown Role");
  });

  it("includes skills and tools from registry", async () => {
    const prompt = await buildSystemPrompt("Sales", mockAnalysis, roster);
  ...
  it("includes collaboration guidelines", async () => {
    const prompt = await buildSystemPrompt("Sales", mockAnalysis, roster);
